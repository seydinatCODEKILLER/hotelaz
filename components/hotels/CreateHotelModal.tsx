...
import { CreateHotelData } from '@/types/hotel'
import { useCreateHotel } from '@/hooks/hotels/useHotels'
...
export function CreateHotelModal({ open, onOpenChange }: CreateHotelModalProps) {
  ...
  const onSubmit = (data: HotelFormData) => {
    const submitData: CreateHotelData = {
      ...data,
      photo: photo || undefined
    }

    createHotel.mutate(submitData, {
      onSuccess: () => {
        onOpenChange(false)
        form.reset()
        setPhoto(null)
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex gap-3 justify-end pt-6 border-t">
              ...
              <Button type="submit" disabled={createHotel.isPending}>
                {createHotel.isPending ? 'Création...' : 'Créer l\'hôtel'}
              </Button>
            </div>
          </form>
        </Form>
